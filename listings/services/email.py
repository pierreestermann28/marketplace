...
from listings.models import Listing, SearchAlert, SearchAlertNotification
...
def dispatch_search_alerts(listing: Listing):
    ...
    for alert in alerts:
        ...
        listing_url = reverse(
            "listing_detail",
            kwargs={"slug": listing.slug or "item", "uuid": listing.id},
        )
        if base_url:
            listing_url = f"{base_url}{listing_url}"
        location_label = (
            listing.city
            or listing.postal_code
            or listing.country_code
